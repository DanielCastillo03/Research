...
class HumanWalk(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        healthy_z_range=(.5, 1.1),
        healthy_patella = (0.12, 0.75),
        height_weight = 2.5,
        ...
        **kwargs,
        ...
        if exclude_current_positions_from_observation:
            observation_space = Box(
                low=-np.inf, high=np.inf, shape=(353,), dtype=np.float64
            )
        else:
            observation_space = Box(
                low=-np.inf, high=np.inf, shape=(353,), dtype=np.float64
        ...
        MujocoEnv.__init__(
            self,
            "/home/daniel/Desktop/Research/Rajagopal2015_converted/human.xml",
            30,
    ...
    def is_healthy(self):
        ...
        is_healthy = (min_z < self.data.xpos[1][2] < max_z and
    ...
    def height_check(self):
        return (
            float(self.is_healthy or self._terminate_when_unhealthy) * (self._height_reward * self.data.xpos[1][2])
            )
