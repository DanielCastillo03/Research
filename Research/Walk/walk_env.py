...
class Walk(gym.Env):
    ...
    def reset_model(self):
        ...
        qpos = np.zeros(len(self.data.qpos), dtype=np.float64)
        qvel = np.zeros(len(self.data.qvel), dtype=np.float64)
        
        for i in range(len(self.state.qpos)):
            self.data.qpos[i] = qpos[i] + uniform(noise_low, noise_high)
            self.data.qvel[i] = qvel[i] + uniform(noise_low, noise_high)
